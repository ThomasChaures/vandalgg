...
<template>
  <aside
    class="bg-slate-950 max-w-[330px] w-full overflow-hidden px-5 py-5 rounded-xl mt-[40px]"
  >
    ...
    <ul v-if="loader" class="h-full flex flex-col gap-5 mt-6">
      ...
      >
        <router-link
          :to="'/perfil/' + user.usertag"
          class="w-full flex items-center gap-3"
        >
     
          <div
            class="img-perfil relative w-[35px] h-[35px] flex items-center justify-center rounded-full overflow-hidden bg-gray-200"
          >
            <img
              v-if="user.photo"
            ...
            <i
              v-else
              class="fa-solid text-[33px] fa-user text-gray-500 absolute bottom-[-4px]"
            ></i>
          </div>
